...
const getFollowingPeopleIdOfUser = async username => {
  ...
  ON user.user_id=follower.follower_user_id WHERE 
...
//API 3 : GET tweets of user API
app.get('/user/tweets/feed/', authonticateToken, async (request, response) => {
...
//API 4 : GET user follows list API
app.get('/user/following/', authonticateToken, async (request, response) => {
  const {username, userId} = request
  const getFollwingUsersQuery = `SELECT name FROM follower
    INNER JOIN user ON user.user_id =follower.following_user_id 
    WHERE follower_user_id='${userId}';`
  const followingPeople = await db.all(getFollwingUsersQuery)
  response.send(followingPeople)
})

//API 5 : GET people follows the user list API
app.get('/user/followers/', authonticateToken, async (request, response) => {
  const {username, userId} = request
...
//API 6 : GET tweet of user follows API
app.get(
  '/tweets/:tweetId/',
  authonticateToken,
...
//API 7 : list of names who liked tweets API
app.get(
  ...
  async (request, response) => {
    ...
    const likeResult = await db.all(getLikesQuery)
    const likes = likeResult.map(user => user.username)
    response.send({likes: likes})
  },
)

// API 8 : list of replies tweet API
app.get(
  ...
  async (request, response) => {
    const {tweetId} = request.params
    const getTweetsQuery = `SELECT name,reply FROM user INNER JOIN reply ON 
    user.user_id=reply.user_id WHERE tweet_id='${tweetId}';`
    const replyResult = await db.all(getTweetsQuery)
    response.send({replies: replyResult})
  },
)

//API 9 : return list of all tweets of user API
app.get('/user/tweets/', authonticateToken, async (request, response) => {
  const {userId} = request
  const getTweetsListQuery = `
    ...
    `
  const tweets = await db.all(getTweetsListQuery)
  response.send(tweets)
})

//API 10 : create tweet API
app.post('/user/tweets/', authonticateToken, async (request, response) => {
...
//API 11 : delete a tweet API
app.delete('/tweets/:tweetId', authonticateToken, async (request, response) => {
  const {tweetId} = request.params
  const {userId} = request
